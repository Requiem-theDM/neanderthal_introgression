...
# Filter data to include only rows with variantPIP > 0.1
filtered_data <- data %>%
  filter(variantPIP > 0.1)

# Count unique occurrences of variant_rsID for each POPGROUP
variant_pop_unique <- filtered_data %>%
  group_by(POPGROUP) %>%
...
ggplot(variant_pop_unique, aes(x = reorder(POPGROUP, -unique_SNPs), y = unique_SNPs, fill = POPGROUP)) +
  ...
  labs(
    x = "Population Group", 
    y = "Number of SNPs",
    title = "Notably Introgressed SNPs per Population Group"
  ) +
  theme_minimal() +
  theme(
  ...
  scale_fill_viridis_d(option = "cividis")  # Using the 'cividis' colorblind-friendly palette
